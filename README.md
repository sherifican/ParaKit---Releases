...
(this is the older .exe version, Open Source repo is newer):` [LimeWire link for v4.4.52](https://limewire.com/d/UV9Zm#DHqxKgEtmn)
...
The goal of this project is to make drum chart creation as accessible as possible -- *NO* paywalls, no juggling multiple audio software and/or free web-services with ads everywhere, lower barrier to entry. ParaKit always has been and always will be free of charge and will *NEVER* host ads. If you've ever gotten tired or frustrated by the some of the unnecessarily convoluted and tedious parts of the process of chart making, or tools forcing a subscription on you, this app is for you. I've condenced all the necessities into one place, no emails or credit cards required.
...
This would be pretty useful for those who want to chart their own personally composed songs, or for those who would rather just actually play the song to chart the notes instead of doing it the traditional listen & click method. I would also of course add MIDI input to the Mini Game as well. Stay tuned.
...
| Version | Summary |
|---------|---------|
| **v4.4.52** | Update check rewritten — The Check for Updates button (top right corner of app) now fetches the LimeWire release page, reads the version from the page title, and prompts you when a newer ParaKit is available; clicking Yes opens your browser to the LimeWire link for manual download. No more silent auto-update or Google Drive routing. Kick clustering tightened in Audio → MIDI — the hybrid-mode ML-only kick gap widened from 45 ms to 60 ms, so dense metal tracks no longer produce visually duplicate kick pairs in the editor; legitimate fast double-kicks at metal tempos are still detected. Waveform display mode "Jagged (Detailed)" renamed to "Jagged Hollow" for clearer language; existing saved config selections are preserved (no reset needed). Help tab gains a new "Send to Song Creator" entry under the MIDI Editor section explaining the full handoff workflow (MIDI + Drums stem auto-copy + "Use BPM from MIDI file" auto-enable) and what you still need to add manually. Plus cosmetic doc cleanups, stale "Track Preview" docstring sweep, and `README for users.txt` removed (the help tab & github README serves in its place). |
